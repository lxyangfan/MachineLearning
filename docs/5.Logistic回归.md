...
> 这里没有详细介绍训练算法的目的，仅仅说为了找到最佳参数，如何为最佳？既然是最佳，需要给出一个最优化问题的描述，这样对于初学者（比如：我 =_=）更友好。以下为我补充的建立最优化问题的过程：

使用 `Sigmoid` 函数用来二分分类，如下：
$$f(z) = \frac{1} {1 + e^{-z}} $$

$$z = w_1x_1 + w_2x_2 + ... + w_nx_n = \mathbf{W}^T\mathbf{X}$$

现在假设我们**已经根据训练数据**得出向量\\(\mathbf{W^T}\\)，对于一个新的数据\\(\mathbf{X_k}\\), 如果\\(f(z_k) >0.5\\)，则分\\(\mathbf{X_k}\\)为类1，否则分为类0；

而分类问题就转化为如何根据训练数据求出**最优**的\\(\mathbf{W^T}\\)。这里我们定义如下问题：
$$arg \min{(h-f(z))} $$
